...
commit_overlay() {
    ...
    local ovrsrc="${TMPDIR}/overlay/statoverride"
    local ovrdst="${TMPDIR}/overlay-statoverride"
    if [ -e "$ovrsrc" ]; then
        sed -e '/^#/d' "$ovrsrc" > "$ovrdst"
        # Don't include the statoverride file in the commit
        rm "$ovrsrc"
    else
        >"$ovrdst"
    fi
    ...
    ostree commit --repo="${tmprepo}" --tree=dir="${TMPDIR}/overlay" -b "${name}" \
        ...
        --statoverride "${TMPDIR}/overlay-statoverride"
}
